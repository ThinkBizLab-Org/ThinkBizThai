...
test('the tables batch 050 adds have their own entries in the CI negative control', async () => {
  ...
  for (const table of ASYNC_TABLES) {
    const resting = cases.filter((c) => flips(c) && (c.covers ?? []).some((x) => String(x).includes(table)));
    assert.ok(resting.length >= 2, `${table}'s control entry rests on ${resting.length} case(s) that `
      + 'change with RLS off; the entry is only evidence if at least two do');
  }
});
...
test('every batch 130 control entry rests on cases that would fail with RLS off', () => {
  const BILLING_TABLES = ['billing_plans', 'billing_plan_prices', 'billing_entitlements', 'billing_subscriptions'];
  for (const [table, floor] of BILLING_TABLES.map((t) => [t, 1])) {
    const entry = controlEntries.find((e) => e[1] === `app.${table}`);
    assert.ok(entry, `app.${table}: the per-family control has no entry for a table this batch creates`);
...
test('the coverage map records what batch 130 could carry and what a workspace row cannot', () => {
  ...
  for (const [key, needle] of [[3, 'BATCH 130 CARRIES NO CASE FOR IT AND SAYS WHY'],
    ...
    [9, 'BATCH 130 ADDS A FIFTH AND A SIXTH IMMUTABLE TABLE'],
