-- Batch 042 — the knowledge service path is closed, by a policy that names every role.
--
-- Owner: A0 Integration / DB-00. Closes the S8 shape in batch 040 (and 041, which adds no table): narrowings `for all to
-- authenticated` that claim to bound every writer and bind no service role. Found not by a role run
-- on 040 (and 041, which adds no table) but by READING, after the 2026-09-15 role runs had found the shape in 081, 090 and 100
...
--
-- WHY A SEPARATE FILE AND NOT AN EDIT TO 040 (and 041, which adds no table): the batch counts its own restrictive policies at
...
-- 042: §6's registry does not reserve it, as it did not reserve 082, 083, 092 or 101.
--
-- Today this changes nothing observable, for 101's reason rather than 082's: batch 040 grants app_worker SELECT, INSERT and UPDATE on app.knowledge_items and SELECT and INSERT on app.knowledge_item_versions, and 041 grants it EXECUTE on app.knowledge_scope_applies
...
create policy knowledge_items_service_path_closed on app.knowledge_items
  as restrictive
  for all
  using (current_user = 'authenticated')
  with check (current_user = 'authenticated');

create policy knowledge_item_versions_service_path_closed on app.knowledge_item_versions
  as restrictive
  for all
  using (current_user = 'authenticated')
  with check (current_user = 'authenticated');

comment on policy knowledge_items_service_path_closed on app.knowledge_items is
  'Batch 042, shape C (batch 082) for the S8 shape in batch 040 (and 041, which adds no table): every role that is not authenticated is '
  'refused every row until a batch amends this policy beside a narrowing that binds the acting user (shape B, RFC-2026-023).';
comment on policy knowledge_item_versions_service_path_closed on app.knowledge_item_versions is
  'Batch 042, shape C (batch 082) for the S8 shape in batch 040 (and 041, which adds no table): every role that is not authenticated is '
  'refused every row until a batch amends this policy beside a narrowing that binds the acting user (shape B, RFC-2026-023).';
...
begin
  ...
  if count_of <> 2 then
    raise exception 'batch 042 finds % service-path closures and there must be one per table, 2', count_of;
  end if;
  ...
  --    batch 040 grants app_worker SELECT, INSERT and UPDATE on app.knowledge_items and SELECT and INSERT on app.knowledge_item_versions, and 041 grants it EXECUTE on app.knowledge_scope_applies, so a service role does reach the point where policies are consulted here. The
  ...
  if offending is not null then
    raise exception 'a permissive policy on a closed table admits a role other than authenticated: %', offending
      using hint = 'Batch 042 is written on the premise that today only authenticated is admitted anywhere in this family; a service policy here needs the shape-B narrowing first, and RFC-2026-022 is not in effect.';
  end if;
