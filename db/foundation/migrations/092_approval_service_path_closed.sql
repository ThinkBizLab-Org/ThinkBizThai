-- Batch 092 — the approval service path is closed, by a policy that names every role.
--
-- Owner: A0 Integration / DB-00. Closes the S8 shape in batch 090 (090_*.sql), found by the
-- independent role runs of 2026-09-15 (A1 S11, C0 H2, Q0 F5) after batch 082 had closed the same shape on
-- batch 080's tables, and integrated in the same pull request as 090 so that 090 never sits
...
-- narrowings in batch 090 are `for all to authenticated` and bind no service role; the three
...
--
-- WHY A SEPARATE FILE AND NOT AN EDIT TO 090: batch 090 counts its own restrictive policies at
-- apply time and its static rules read its own text; a closure written into it would trip both and
-- rewrite what three role runs reviewed. A forward file after 090 leaves 090 as reviewed and
-- is the shape 082 set. It is numbered 092: §6's registry does not reserve it, as it did not
-- reserve 082.
--
...
create policy approval_policies_service_path_closed on app.approval_policies
  as restrictive
  for all
  using (current_user = 'authenticated')
  with check (current_user = 'authenticated');

create policy approval_requests_service_path_closed on app.approval_requests
  as restrictive
  for all
  using (current_user = 'authenticated')
  with check (current_user = 'authenticated');

create policy approval_events_service_path_closed on app.approval_events
  as restrictive
  for all
  using (current_user = 'authenticated')
  with check (current_user = 'authenticated');

comment on policy approval_policies_service_path_closed on app.approval_policies is
  'Batch 092, shape C (batch 082) for the S8 shape in batch 090: every role that is not authenticated is '
  'refused every row until a batch amends this policy beside a narrowing that binds the acting user (shape B, RFC owed).';
comment on policy approval_requests_service_path_closed on app.approval_requests is
  'Batch 092, shape C (batch 082) for the S8 shape in batch 090: every role that is not authenticated is '
  'refused every row until a batch amends this policy beside a narrowing that binds the acting user (shape B, RFC owed).';
comment on policy approval_events_service_path_closed on app.approval_events is
  'Batch 092, shape C (batch 082) for the S8 shape in batch 090: every role that is not authenticated is '
  'refused every row until a batch amends this policy beside a narrowing that binds the acting user (shape B, RFC owed).';
...
declare
  ...
  count_of   integer;
  probe      record;
  closed_tables constant text[] := array['approval_policies', 'approval_requests', 'approval_events'];
...
begin
  ...
  if count_of <> 3 then
    raise exception 'batch 092 finds % service-path closures and there must be one per table, 3', count_of;
  end if;
  ...
  if offending is not null then
    raise exception 'a role this batch closes the path for already holds a privilege: %', offending
      using hint = 'Batch 092 is written on the premise that it changes nothing today. If a grant was made, the batch that made it owes the shape-B narrowing first.';
  end if;
