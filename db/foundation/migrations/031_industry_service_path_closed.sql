-- Batch 031 — the industry service path is closed, by a policy that names every role.
--
-- Owner: A0 Integration / DB-00. Closes the S8 shape in batch 030: narrowings `for all to
-- authenticated` that claim to bound every writer and bind no service role. Found not by a role run
-- on 030 but by READING, after the 2026-09-15 role runs had found the shape in 081, 090 and 100
...
--
-- WHY A SEPARATE FILE AND NOT AN EDIT TO 030: the batch counts its own restrictive policies at
...
-- 031: §6's registry does not reserve it, as it did not reserve 082, 083, 092 or 101.
--
-- Today this changes nothing observable, for 101's reason rather than 082's: batch 030 grants app_worker SELECT, INSERT and UPDATE on app.industry_assignments
...
--
-- LEFT OPEN ON PURPOSE: app.industry_packs and app.industry_pack_versions are GLOBAL tables that carry no narrowing -- they make no S8 claim, and the map records a closure there as a separate question.

create policy industry_assignments_service_path_closed on app.industry_assignments
  as restrictive
  for all
  using (current_user = 'authenticated')
  with check (current_user = 'authenticated');

comment on policy industry_assignments_service_path_closed on app.industry_assignments is
  'Batch 031, shape C (batch 082) for the S8 shape in batch 030: every role that is not authenticated is '
...
declare
  ...
  count_of   integer;
  probe      record;
  closed_tables constant text[] := array['industry_assignments'];
...
begin
  ...
  if count_of <> 1 then
    raise exception 'batch 031 finds % service-path closures and there must be one per table, 1', count_of;
  end if;
  ...
  --    batch 030 grants app_worker SELECT, INSERT and UPDATE on app.industry_assignments, so a service role does reach the point where policies are consulted here. The
  ...
  if offending is not null then
    raise exception 'a permissive policy on a closed table admits a role other than authenticated: %', offending
      using hint = 'Batch 031 is written on the premise that today only authenticated is admitted anywhere in this family; a service policy here needs the shape-B narrowing first, and RFC-2026-022 is not in effect.';
  end if;
