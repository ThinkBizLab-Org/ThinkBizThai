-- Batch 062 — the metering service path is closed, by a policy that names every role.
--
-- Owner: A0 Integration / DB-00. Closes the S8 shape in batch 061: narrowings `for all to
...
--
-- WHY A SEPARATE FILE AND NOT AN EDIT TO 061: the batch counts its own restrictive policies at
...
comment on policy quota_buckets_service_path_closed on app.quota_buckets is
  'Batch 062, shape C (batch 082) for the S8 shape in batch 061: every role that is not authenticated is '
...
declare
  ...
  count_of   integer;
  probe      record;
  closed_tables constant text[] := array['quota_buckets'];
begin
  ...
  if count_of <> 1 then
    raise exception 'batch 062 finds % service-path closures and there must be one per table, 1', count_of;
  end if;
  ...
  --    batch 061 grants app_worker SELECT, INSERT and UPDATE (column-scoped) on app.quota_buckets, so a service role does reach the point where policies are consulted here. The
  ...
  if offending is not null then
    raise exception 'a permissive policy on a closed table admits a role other than authenticated: %', offending
      using hint = 'Batch 062 is written on the premise that today only authenticated is admitted anywhere in this family; a service policy here needs the shape-B narrowing first, and RFC-2026-022 is not in effect.';
  end if;
