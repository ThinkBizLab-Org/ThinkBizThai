...
|---|---|---|---|---|---|
| business/page (020, 021) | `business_profiles`, `business_profile_versions`, `page_context_profiles`, `page_context_profile_versions` | SELECT on all four (grants, no policy) | none | a worker read that no policy admits today anyway | **closable now**, 082's shape |
| industry (030) | `industry_assignments` | SELECT on `industry_packs`, `industry_pack_versions`, `industry_assignments` | none | as above | **closable now** (packs and pack versions are global tables and carry no narrowing — a closure there is a separate question) |
| knowledge (040, 041) | `knowledge_items`, `knowledge_item_versions` | SELECT on both | none | as above | **closable now** |
