...
| Cross-vendor manifest-to-handoff dry run | `partial; blocked-environment` | [Claude Code external dry-run evidence](WP-0A-A0-001/cross-vendor-claude-code-dry-run.md) records a genuine Anthropic Claude Code read-only run against base `bb11ccb`: it read the real manifest/inputs and returned a protocol-shaped report, but `npm run check` stopped at the exact Node guard (required `v24.20.0`, observed `v26.7.0`). This is not external test-pass/G0/merge evidence and does not close the item; a distinct-vendor pinned-runtime replay remains required. |
