...
> **Corrected after independent testing, 2026-09-03.** This paragraph was right and the `tests[]` entry in the handoff was not: it said the five holes were closed *"and each was mutation-tested"*, listing the `$ref` one among them. **Two attestations by the same author disagreed inside one file, and the one that claimed more was the wrong one.** The handoff string is corrected; this limitation is not weakened to meet it. The Tester subsequently forced the branch in a disposable clone by rewriting a `$ref` to an absolute URL and reports it fires with its own distinct reason — **that is the Tester's evidence and is recorded as theirs.** An author does not get to cite someone else's execution as his own coverage, which is the whole reason the two sentences had to be reconciled downward rather than upward.

### Three checker limitations independent testing found, recorded rather than fixed

The Tester ruled none of these blocking and recommended writing them down so the next author does not rediscover them. They are limitations of the checker, not defects in the dictionary.

1. **`source.status` is never validated against the document's own `source_status_vocabulary`.** A metric whose status reads `"obviously_derivable"` — a word this dictionary defines nowhere — exits 0. This is the sharpest of the three, because **the entire A-03 argument turns on what a status word means**, and the three defined statuses route work: `absent` to a contract owner, `derivable_with_caveat` to A6. A status outside the vocabulary routes nothing and nothing says so. Closing it means checking every `source.status` against the vocabulary's keys — minus `vocabulary_amendment_note`, which is prose sitting in that object and would have to be excluded.
2. **The field-versus-formula check is one-directional.** It fails when a formula *names* a field the entry does not declare, and never when an entry *declares* a field its formula does not name: a formula gutted to a single word, naming none of its seven declared fields, exits 0. And worse for the very defect class it was built for — a token that does not resolve is silently treated as prose, so **had C-03's original defect been a misspelling of `occurred_at` rather than an omission of it, hole 5 as built would not have caught it.** A misspelling does not resolve, and an unresolvable token is exactly what prose looks like to this check. Closing it needs the reverse direction plus a way to tell a misspelled field from an English word — which the backtick convention could carry and does not yet.
3. **The pin is a count, and "PINNED" reads stronger than it is.** `EXPECTED_RESOLVED_PATHS` catches a citation that leaves and one that arrives, and **not a compensating pair**: remove one and add another and the total is still 63 and the run is still green. The script's header calls it a count and claims nothing more, so nothing false was written — but it is recorded here so the next author does not read it as a checksum over the citations. A digest of the sorted field list would close it. *(The word `PINNED` in the handoff's evidence line is lowercased to `pinned` in the same change, for the same reason.)*
