...
10. **The environment the interpreter starts in is not defensible from inside the repository.** `NODE_OPTIONS=--require …` runs code before any guard and exits 0. So does anything else the runner is handed. The answer is protected CI on a trusted runner — item 3 — not another test. Stated rather than guarded badly.
